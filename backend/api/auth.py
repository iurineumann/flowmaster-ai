# backend/api/auth.py (NOVO MÓDULO DE AUTENTICAÇÃO COM ENTRA ID)
...
from ..utils.security import create_token, authenticate_user_entra_id, authenticate_user # Mantém o authenticate_user para fallback/teste
...
async def login_via_entra_id(
    ...
    user = authenticate_user_entra_id(request.entra_id_token, db)
    
    if not user:
        # A função authenticate_user_entra_id deve levantar a exceção, mas esta é uma segurança
        raise HTTPException(
